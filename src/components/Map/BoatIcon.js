...
export const Boat = ({ index, className, height, width, viewBox, rotation, color, trailColor }) => {
  const defaultColors = getColorVariation(index);
  const boatColor = !color ? defaultColors[0] : color;
  const boatTrailColor = !trailColor ? defaultColors[1] : trailColor;
